...
def test_graph_crud_flow(client: TestClient):
    ...
    update_course = client.patch(
        ...
        json={"status": "completed", "grade": "3.7"},
    )
    ...
    assert len(detail_resp.json()["courses"]) == 2

    export_resp = client.post(f"/api/v1/graphs/{graph_id}/export")
    assert export_resp.status_code == 200
    assert export_resp.json()["graph"]["title"] == "My Plan"
...
def test_export_import_replace(client: TestClient):
    graph_resp = client.post("/api/v1/graphs", json={"title": "ImportTest"})
    ...
    import_resp = client.post(
        f"/api/v1/graphs/{graph_id}/import",
        json={
            "replace_existing": True,
            "courses": [
                {"code": "NEW101", "title": "New Course", "credits": 2},
                {"code": "NEW102", "title": "Another", "credits": 3},
            ],
        },
    )
    ...
    list_courses = client.get(f"/api/v1/graphs/{graph_id}/courses")
    assert len(list_courses.json()) == 2
...
def test_template_publish_and_clone(client: TestClient):
    ...
    assert clone_resp.json()["id"] != graph_id
